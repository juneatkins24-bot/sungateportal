...
function Media({ edit }) {
  const a = edit.aspect || (edit.kind === "image" ? "square" : "phone");
  ...
  if (edit.kind === "video" && edit.ready) {
    ...
    if (a === "phone") {
      return (
        <div className="pe-frame phone">
          <span className="b l" /><span className="b l2" /><span className="b r" />
          <div className="screen">{inner}</div>
        </div>
      );
    }
    return (
      <div className={`pe-frame ${a === "landscape" ? "landscape" : "square"}`}>
        {inner}
        ...
        <div className="pe-edits">
          {edits.map((edit, i) => (
            <div className="pe-edit" key={edit.id}>
              ...
              <div className="title">{edit.title || edit.file}</div>
              <Media edit={edit} />
