    ...
    <main className="admin">
      ...
      <section style={{ marginTop: 44 }}>
        ...
        <form onSubmit={addArtist}>
          ...
          <button className="pill ghost" type="submit" disabled={busy}>Add artist</button>
        </form>
      </section>
      ...
      <section style={{ marginTop: 44 }}>
        ...
        {data.assets.slice(0, 20).map((x) => (
          <div className="asset-li" key={x.id}>
            <span>{x.file}</span>
            <span className={`st ${x.streamUid ? "ready" : "processing"}`}>{x.streamUid ? "ready" : "processing"}</span>
          </div>
        ))}
      </section>
    </main>
